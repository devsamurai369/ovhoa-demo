...
├── rinks.html              # rink locations (Rinks sheet tab)
├── rule-books.html
...
**Tab names must match exactly** (`Meetings`, `Board`, `Banner`, `Rinks`,
and optionally `Season`). If a tab is renamed or missing, Google quietly
...
### Rink locations (Rinks tab)

The **Rink Locations** page is driven by a **`Rinks`** tab with
`Name | Address | Notes` columns. Each rink renders as a card with a
tap-to-navigate **Directions** button built from its name and address, which
is the useful bit on a phone. Until the tab exists, the page shows a
"coming soon" message rather than an empty screen.
