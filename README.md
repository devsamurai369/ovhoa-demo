...
├── rinks.html              # rink locations (static content in build.py)
├── rule-books.html
├── scorekeepers-guide.html
├── 404.html                # generated by build.py (Netlify serves it automatically)
...
├── .htaccess               # only used if hosted on Apache/LiteSpeed
└── assets/
...
**Tab names must match exactly** (`Meetings`, `Board`, `Banner`, and
optionally `Season`). If a tab is renamed or missing, Google quietly
returns the *first* tab instead of an error — the site detects this by
checking the header row and ignores the data rather than, say, listing your
meeting schedule as rink locations.
...
### Announcement banner (Banner tab)

An orange strip at the top of the homepage, driven by a **`Banner`** tab with
`Header | Text` columns (an optional `Author` column becomes a by-line).

Leave the row blank and the banner disappears entirely — that is the "off"
switch, so no code change is needed to take a notice down. Only the first row
with content is shown.

### Rink locations (static)

The **Rink Locations** page is deliberately *not* sheet-driven — rink
addresses change rarely, and a wrong address sends an official to the wrong
building. Edit the `RINKS` block in `build.py` and rebuild to add or correct a
...
The site is prebuilt — `build.py` runs on a developer machine and the
generated `.html` is committed — so Netlify has **no build step**. Settings
live in `netlify.toml`; a deploy can never fail over a build.

**First-time setup**

1. Netlify → **Add new site → Import an existing project → GitHub**, pick
   `devsamurai369/ovhoa-demo`.
2. Branch to deploy: `main` for production (`Test-Branch` gets its own
   preview URL automatically — see below). Build command: **leave empty**.
   Publish directory: `.`
3. **Domain → Add a custom domain → `ovhoarefs.com`**, then point DNS at
   Netlify (either move the nameservers, or add the `A`/`CNAME` records
   Netlify shows you).
4. Netlify issues and auto-renews a free Let's Encrypt certificate, which
   permanently fixes the expired-certificate problem the old Hostinger site
   had. Turn on **Force HTTPS** once the certificate is issued.

**Every push deploys.** Pushing to the production branch publishes the site.
Pushing to any other branch (including `Test-Branch`) gets its own preview URL
— which is a clean way to review a change on a phone, or on a second machine,
before it goes live.

**Rolling back.** Netlify → Deploys → pick an earlier deploy → *Publish
deploy*. Instant, and worth knowing before the board starts editing.

Note that `publish = "."` also serves `build.py` and `README.md` at their
URLs. Neither holds anything private (the Google Sheet ID is already public in
`assets/js/sheets.js`, and the sheet itself is a read-only public link), so
this is harmless — but don't put anything secret in this repo.

### If the site ever moves to Apache/LiteSpeed hosting instead

The repo also carries a `.htaccess` with `ErrorDocument 404 /404.html`, which
is what Hostinger needs to serve the 404 page. Netlify ignores it and picks up
`404.html` on its own. If you deploy to a server that already has a
`.htaccess`, merge that one line into the existing file rather than
overwriting it.
